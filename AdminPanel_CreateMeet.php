<?php
	...
	//MEET NAME CAN HAVE DUPLICATES, FOR EXAMPLE ANNUAL EVENTS
		...
		$params = array(':meet_name' => $meetname, ':start_date' => $startdate);
		$stmt->execute($params);
		$row = $stmt->fetch();
	if ($row != '0'){
	...
	//CREATE NEW MEET
	$query = $db->prepare("INSERT INTO Meets (meet_name, start_date, end_date, capacity, zip_code, city, state, street_address) VALUES (?, ?, ?, ?, ?, ?, ?, ?");
	$query->execute(array($meetname , $startdate_convert, $enddate_convert, $capacity, $zip_code, $city, $state, $street_address));
	
	//SEND EMAIL TO ALL COACHES ABOUT THE NEW MEET
	$query = $db->prepare("SELECT email FROM Coaches");
	$query->execute();
